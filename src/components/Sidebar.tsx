...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-80 bg-zinc-950 border-r border-zinc-800/80 flex flex-col justify-between shrink-0 min-h-screen sticky top-0 h-screen overflow-y-auto">
      <div className="p-6 space-y-7">
        {/* Brand Header */}
        <Logo
          className="flex items-center gap-3.5 px-1 py-1 mb-3"
          iconClassName="w-12 h-12 shrink-0"
          textClassName="text-white text-xl font-bold tracking-tight"
          subtextClassName="text-[11px] text-zinc-400 font-sans block leading-tight mt-0.5"
        />
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-zinc-900/90 border border-zinc-800 rounded-xl pl-10 pr-3.5 py-2.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-red-500/60 focus:ring-1 focus:ring-red-500/60 font-mono transition-all"
          />
        </div>

        {/* Main Navigation */}
        <nav className="space-y-2">
          <div className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest px-1 mb-3 font-mono">
            Navigation Menu
          </div>
          ...
          >
            ...
            {selectedId && (
              <span className="text-[10px] font-mono bg-red-950 text-red-400 px-1.5 py-0.5 rounded border border-red-800/40 truncate max-w-[90px]">
                {selectedId}
              </span>
            )}
          </button>
            ...
            {seedKeys.map((key) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 rounded-lg text-xs font-mono transition-all flex items-center justify-between border ${
                    isSelected
                ...
                >
                  ...
                  {key === "INV-2026-GOLD-99" && (
                    <span className="text-[9px] bg-red-950 text-red-400 px-1.5 py-0.5 rounded border border-red-800/40 shrink-0 font-sans font-semibold">
                      DEMO
                    </span>
                  )}
                </button>
      ...
      <div className="p-6 border-t border-zinc-800/80 space-y-3.5 bg-zinc-950/80">
        <button
          onClick={onReevaluateAll}
          disabled={isEvaluating}
          className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-red-600/10 hover:bg-red-600/20 text-red-400 border border-red-500/30 text-xs font-mono font-semibold transition-all disabled:opacity-50"
        >
